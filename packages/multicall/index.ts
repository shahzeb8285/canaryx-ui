...
export const multicallAddresses = {
  ...
  19: '0x87aE766dB42397F0F123bf857634e8792440DD53',
}
...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.SONGBIRD) => {
    console.log("createMulticall")

    const multi = getMulticallContract(chainId, provider({ chainId }))
    console.log("createMulticall",multi)
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.SONGBIRD, options, provider: _provider }) => {
