...
const startEndBlockCalls = livePoolsWithEnd.flatMap((poolConfig) => {
  return [
    {
      address: getAddress(poolConfig.contractAddress,ChainId.SONGBIRD),
      name: 'startBlock',
    },
    {
      address: getAddress(poolConfig.contractAddress,ChainId.SONGBIRD),
...
const poolsBalanceOf = poolsConfig.map((poolConfig) => {
  return {
    ...
    name: 'balanceOf',
    params: [getAddress(poolConfig.contractAddress,ChainId.SONGBIRD)],
  }
})

export const fetchPoolsTotalStaking = async () => {
  const poolsTotalStaked = await multicall(erc20ABI, poolsBalanceOf,ChainId.SONGBIRD)
...
export const fetchPoolsStakingLimits = async (
  ...
  const poolStakingCalls = validPools
    .map((validPool) => {
      const contractAddress = getAddress(validPool.contractAddress,ChainId.SONGBIRD)
